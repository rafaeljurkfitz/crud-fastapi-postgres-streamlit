...
st.header("JWT Test Application")

# Your API url to get JWT token
api_url = "http://backend:8000/login/access-token"
...
authenticator = Authenticator(
    ...
    cookie_lifetime=timedelta(minutes=60)
)
...
if st.session_state["authentication_status"]:
    ...
    with st.expander("Adicionar um Novo Produto"):
        with st.form("new_product"):
            ...
            email_provider = st.text_input("Email do Fornecedor")
    ...
    with st.expander("Atualizar Produto"):
        with st.form("update_product"):
            ...
            if update_button:
                ...
                if new_email:
                    update_data["email_provider"] = new_email
                if new_categoria:
